    ...
    <form action="<?= url_to('login') ?>" method="POST">
        ...
        <div class="text-sm text-center leading-loose">
            <?php if (setting('Auth.allowMagicLinkLogins')) : ?>
                <p class="text-center"><?= lang('Auth.forgotPassword') ?> <a href="<?= url_to('magic-link') ?>" class="decoration-2 decoration-primary underline hover:no-underline"><?= lang('Auth.useMagicLink') ?></a></p>
            <?php endif ?>
            
            <?php if (setting('Auth.allowRegistration')) : ?>
                <p class="text-center"><?= lang('Auth.needAccount') ?> <a href="<?= url_to('register') ?>" class="decoration-2 decoration-primary underline hover:no-underline"><?= lang('Auth.register') ?></a></p>
            <?php endif ?>
        </div>
    </form>
<?= $this->endSection() ?>
